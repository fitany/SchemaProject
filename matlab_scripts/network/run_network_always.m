function network = run_network_always(network,params,is_learning,t_per_pair,has_hipp,sigm,disp_on)
    ...
    for t = 1:t_per_pair
        %Update multimodal neurons (only if both well and flavor inputs are
        %available)
        if isequal(n_flavor,zeros(params.size_flavors,1)) || isequal(n_well,zeros(params.size_wells,1))
            n_multimodal_new = network.n_multimodal;
        else
            if sigm
        ...
        if has_hipp
            if sigm
                n_buffer_new = 2.*sigmoid(.99 .* n_buffer + n_multimodal + 0.1*w_buffer_hipp'*n_hipp,gain)-1; %activation
            else
                n_buffer_new = max(.99 .* n_buffer + n_multimodal +  0.1*w_buffer_hipp'*n_hipp,0); %activation
            end
        else
            if sigm
