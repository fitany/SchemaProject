% Adds many bidirectional connections, attempts to run the network as the
% agent explores an arena, then input a single flavor and see the resulting
% location in n_well.

% Currently not working
function network = run_network_retrieval_complex(n_well,n_flavor,network,params,is_learning,t_per_pair,has_hipp,sigm,disp_on)
    ...
    %Testing out the use of this area
    ...
    for t = 1:t_per_pair
        %Update input
        if sigm
            n_input_new = 2.*sigmoid(input_current + w_input_multimodal'*n_multimodal,gain)-1; %activation
        else
            n_input_new = max(input_current + w_input_multimodal'*n_multimodal,0); %activation
        end
        n_input_new(n_input_new < .15) = 0; % voltage gating
        %Update multimodal neurons
        if sigm
            n_multimodal_new = 2.*sigmoid(w_input_multimodal*n_input + .5.*n_buffer,gain)-1; %activation
        else
            n_multimodal_new = max(w_input_multimodal*n_input + .5.*n_buffer,0); %activation
        end
        ...
        if has_hipp
            if sigm
                n_buffer_new = 2.*sigmoid(.99 .* n_buffer + n_multimodal + w_buffer_hipp'*n_hipp + w_buffer_pfc'*n_pfc,gain)-1; %activation
            else
                n_buffer_new = max(.99 .* n_buffer + n_multimodal +  w_buffer_hipp'*n_hipp + w_buffer_pfc'*n_pfc,0); %activation
            end
        else
            if sigm
                n_buffer_new = 2.*sigmoid(.99 .* n_buffer + n_multimodal + w_buffer_pfc'*n_pfc,gain)-1; %activation
            else
                n_buffer_new = max(.99 .* n_buffer + n_multimodal + w_buffer_pfc'*n_pfc,0); %activation
            end
        end
        n_buffer_new(n_buffer_new < .3) = 0; % voltage gating
        %Update pfc neurons
        if has_hipp
            if sigm
        ...
        if disp_on
            ...
            title('n input');
            subplot(3,3,2);
            imagesc(n_multimodal_new);
            title('n multimodal');
            subplot(3,3,3)
            imagesc(n_buffer_new);
            title('n buffer');
            subplot(3,3,4)
            imagesc(n_pfc_new);
            title('n pfc');
